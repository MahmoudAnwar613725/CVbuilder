...
class WorkDialog ( ) : DialogFragment() {
    private lateinit var workListner:  AddworkListner
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        val dialog = activity?.let {
            val builder = AlertDialog.Builder(it)
            val inflater = requireActivity().layoutInflater
            val view = inflater.inflate(R.layout.dialog_add_work, null)
            ...
            builder.setView(view).apply {
                ...
                view.findViewById<Button>(R.id.btn_add)?.setOnClickListener {
                    ...
                    workListner.onAddWOrk(Work(title, position, duration, R.drawable.ic_work_placeholder))
                    dismiss()
                }
    ...
    fun setWorlListner(addworkListner: AddworkListner){
        workListner = addworkListner
    }
    ...
    override fun onAttach(context: Context) {
       ...
       // workListner = activity as AddworkListner
    }
}